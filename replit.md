...
The project is prepared for migration to **Render** via a Blueprint (`render.yaml` at the repo root) that provisions three resources:

- **`tiempolibre-db`** — Render-managed PostgreSQL (Postgres 16, plan `basic-256mb`, region `oregon`).
- **`tiempolibre-api`** — Render Web Service (Docker), built from `artifacts/api-server/Dockerfile` (multi-stage Node 22 + esbuild bundle; runtime is just `dist/index.mjs`). Health probe at `/healthz`. `DATABASE_URL` injected via `fromDatabase`. `SESSION_SECRET` auto-generated by Render. `CORS_ORIGIN` is `sync: false` (set in dashboard after first deploy with the SPA's public URL).
- **`tiempolibre-web`** — Render Static Site (no Docker for the SPA): `pnpm install --frozen-lockfile --filter "@workspace/delivery-saas..."` + `pnpm --filter @workspace/delivery-saas run build`, serves `artifacts/delivery-saas/dist`. SPA fallback via `routes: rewrite /* → /index.html`. Hashed `/assets/*` cached `immutable`, HTML shell `no-cache`. `VITE_API_BASE_URL` is `sync: false` (set in dashboard with the API's public URL, then trigger a manual redeploy so Vite re-bakes the bundle).

Production-only behavior baked into the API (platform-agnostic; works on Render, Cloud Run, Fly, etc.):

- `app.set("trust proxy", 1)` and `/healthz` endpoint outside `/api` (consumed by Render's `healthCheckPath`).
- `CORS_ORIGIN` env var: comma-separated origin allowlist. Fail-closed in production: the API throws at startup if `CORS_ORIGIN` is missing while `NODE_ENV=production`.
- `csrfOriginGuard` middleware: rejects state-changing requests (non-GET/HEAD/OPTIONS) whose `Origin`/`Referer` is not in the CORS allowlist. Disabled in dev (when `CORS_ORIGIN` is unset) so curl/Postman/Replit proxy keep working.
...
- `lib/api-client-react/src/custom-fetch.ts` defaults `credentials: "include"` so the cookie is sent on every API call (overridable per-request).
- `artifacts/delivery-saas/src/main.tsx` calls `setBaseUrl(import.meta.env.VITE_API_BASE_URL)` at boot. The Vite build inlines this string, so changing it on Render requires triggering a Static Site redeploy.

Docs: `RENDER.md` is the step-by-step Spanish deployment guide (Blueprint creation, post-deploy env wiring, custom domains, Drizzle migrations, troubleshooting).

Removed in the migration: the previous Cloud Run setup (`cloudbuild.yaml`, `CLOUD_RUN.md`, `artifacts/delivery-saas/Dockerfile`, `artifacts/delivery-saas/nginx.conf.template`, `.dockerignore`/`.gcloudignore` of the SPA) — Render serves the SPA as a Static Site so nginx is no longer needed.
