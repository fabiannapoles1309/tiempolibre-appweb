...
router.get("/zones/geojson", requireAuth, requireRole("ADMIN"), (_req, res): void => {
...
router.get("/zones/map-status", requireAuth, requireRole("ADMIN"), (_req, res): void => {
  res.json(getMapStatus());
});

// Recarga forzada del KML (sin reiniciar el servidor).
router.post("/zones/reload", requireAuth, requireRole("ADMIN"), (_req, res): void => {
...
// Disponible para ADMIN y CLIENTE para que puedan previsualizar zona antes de crear pedido.
router.get("/zones/validate", requireAuth, requireRole("ADMIN", "CLIENTE"), async (req, res): Promise<void> => {
